"use strict";
{
	...
	C3.Behaviors.aekiro_scrollView = class aekiro_scrollViewBehavior extends globalThis.ISDKBehaviorBase
	{
		constructor() {
			super();
			...
			this._onWheelHandler = (a) => this._OnMouseWheel(a);
			...
			this.runtime.addEventListener("wheel", this._onWheelHandler);
		}
		_release() {
			...
			this.runtime.removeEventListener("wheel", this._onWheelHandler);
		...
		_OnMouseWheel(a) {
			this._OnMouseWheel2(a["deltaY"], a["clientX"], a["clientY"])
		}
		async _OnInputDown(source, b, c) {
			const insts = this.getAllInstances();
			for (const inst of insts) {
				const beh = globalThis.Aekiro.getInstanceData(inst).aekiro_scrollView;
				if(!beh) continue;
				const wi = inst,
				layer = wi.GetLayer(),
